...
`;
    ...
    console.error("   ⚠️ Error setting up Git structure:", error.message);
  }
}
...
async function pushToGit(reportPath: string, reportSummary: any): Promise<void> {
  ...
  try {
    ...
    const commitMessage = `test: Balance report ${timestamp}
...
Generated by: dummyBalance.ts`;
    ...
    console.log("      git add logs/ && git commit -m 'Add balance report' && git push origin main");
  }
}
...
async function moveOldReports(): Promise<void> {
  try {
    ...
    console.error("   ⚠️ Error moving old reports:", error.message);
  }
}
